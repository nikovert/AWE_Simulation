...
function extraOuts = plotHJIPDE(g, data, extraArgs)
    ...
    if isfield(extraArgs.visualize, 'initialValueSet') &&...
        ...
        set(extraOuts.hVS0, 'DisplayName','zero level-set of V(x,0)','LineWidth',1,...
            'LineColor',[0 1 0],...
            'LevelList',0)
...
%---Visualize Initial Value Function-----------------------------------
    if isfield(extraArgs.visualize, 'initialValueFunction') &&...
        ...
        set(extraOuts.hVF0, 'DisplayName','V(x,0)',...
            'Parent',axes1,...
            'FaceLighting','phong',...
            'FaceAlpha',0.5,...
            'FaceColor',[0 1 0],...
    ...
    % Create ylabel
    ylabel('$\gamma_a$','Interpreter','latex');

    % Create xlabel
    xlabel({'$\chi_a$'},'Interpreter','latex');

    % Create title
    title('t = -0.1 s','Interpreter','latex');

    view(axes1,[127.843277597679 13.7126864167186]);
    axis(axes1,'tight');
    axis(axes1,'square');
    hold(axes1,'off');
    ...
    set(axes1,'FontSize',15,'GridAlpha',0.6,'GridLineStyle','--',...
        ...
        'YTick', [-pi/3 -pi/6 0 pi/6 pi/3],...
        'YTickLabel',{'$-\frac{\pi}{3}$','$-\frac{\pi}{6}$','$0$','$\frac{\pi}{6}$','$\frac{\pi}{3}$'},...
        'YMinorGrid','on','ZGrid','on');
    % Create legend
    legend1 = legend(axes1,'show');
    set(legend1,...
        'Position',[0.00817347258065449 0.770952377659942 0.445804122162168 0.218809530394418],...
        'Interpreter','latex',...
