...
% :Adapted from: Sylvia Herbert (sylvia.lee.herbert@gmail.com)
...
%                       Its subfields are:
%     .postTimestep         (function) This option allows the user to
%                           specify a function which will be called after 
%                           each timestep; the function must conform to the 
%                           PostTimestepFunc prototype of the LSM-toolbox.
...
if isfield(extraArgs, 'small')
    small = extraArgs.small;
else
    small = 1e-4;
end

if small > tau(2)-tau(1)
    warning('dt of tau is smaller than time increment')
end

g = schemeData.grid;
if isfield(extraArgs, 'gDim')
    gDim = extraArgs.gDim;
else
    gDim = g.dim;
end
...
if (isfield(extraArgs, 'visualize') && isstruct(extraArgs.visualize))...
    ...
    title(['t = ' num2str(0) ' s'], 'interpreter','latex')
    set(gcf,'Color','white')
...
dissType = 'local';
if isfield(schemeData, 'dissType')
    dissType = schemeData.dissType;
end
...
if isfield(extraArgs, 'factorCFL')
    integratorOptions = odeCFLset(integratorOptions, ...
                              'factorCFL', extraArgs.factorCFL);
end
startTime = cputime;

% Add custom user postTimestep function
if isfield(extraArgs, 'postTimestep')
    integratorOptions = odeCFLset(integratorOptions, ...
                              'postTimestep', extraArgs.postTimestep);
end
...
for i = istart:length(tau)
    ...
    while tNow < tau(i) - small
        ...
        if ~quiet
            fprintf('  Computing [%f %f]...', tNow, tau(i))
        end
        ...
        delta_t = tNow - tNow_old;
        remaining_t = tau(end) - tNow;
        remaining_timesteps = remaining_t/delta_t;
        remaining_time = remaining_timesteps*computationTime;
        
        if ~quiet
            fprintf('  esitmated time remaining: %f hours \n', remaining_time/3600)
        end
...
extraOuts.executionTime = endTime - startTime;
